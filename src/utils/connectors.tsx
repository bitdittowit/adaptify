...
function createCurvedPathD(points: Coordinates[]): string {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        path.push(`C ${controlPointX1},${controlPointY1} ${controlPointX2},${controlPointY2} ${next.x},${next.y}`);
    }
...
export const createPath = (
    ...
    const { pathClassName, activePathClassName, activeBackPathClassName } = options;
    ...
    const pathElement = document.querySelector<SVGPathElement>('#path');
    const activePathElement = document.querySelector<SVGPathElement>('#path-active');
    ...
    return (
        <>
            <path id="path" d={pathD} className={`${pathClassName}`} />
