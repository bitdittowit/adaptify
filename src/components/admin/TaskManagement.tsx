...
import { RefreshCw, Search, SlidersHorizontal, XIcon, AlertCircle } from 'lucide-react';
...
export function TaskManagement() {
  ...
  useEffect(() => {
    ...
    const fuse = new Fuse(tasks, {
      keys: [
        ...
        { 
          name: "id", 
          getFn: (obj: AdminTask) => String(obj.id),
          weight: 1 
        }
      ],
      threshold: 0.4,
  ...
  const columns = useMemo<ColumnDef<AdminTask>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">
          {row.original.required ? t('required.yes') : t('required.no')}
        </div>
      ),
      size: 80,
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">{row.original.duration_minutes} {t('table.minutes')}</div>
      ),
      size: 100,
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-center">
          {row.original.deadline_days !== null 
            ? t('table.daysAfterArrival', { days: row.original.deadline_days }) 
            : t('table.noDeadline')}
        </div>
      ),
      size: 120,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-wrap justify-center gap-1">
          {row.original.tags && row.original.tags.map((tag, i) => (
