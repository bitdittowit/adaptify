...
export function UserManagement() {
  ...
  const getCountryName = (countryCode: string): string => {
    if (!countryCode) return String(t('filters.emptyValue'));
  ...
  const applyFilters = (usersToFilter: User[], currentFilters: Filters) => {
    ...
    if (currentFilters.role.length > 0) {
      filtered = filtered.filter(user =>
        currentFilters.role.includes(user.role),
      );
    }
        ...
        <CardContent>
          <div className="p-4 border border-red-200 rounded-md bg-red-50 mb-4">
            ...
            <p className="text-red-700">
              {isAuthError ? String(t('errorPermission')) : apiError.message}
            </p>
          </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={checked => {
                          if (checked) toggleFilterValue('sex', 'male');
                          else toggleFilterValue('sex', 'male');
                        }}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={checked => {
                          if (checked) toggleFilterValue('sex', 'female');
                          else toggleFilterValue('sex', 'female');
                        }}
                      />
                      <label
                        htmlFor="female"
                    ...
                    {getUniqueCountries().map(country => (
                      ...
                      >
                        <Checkbox
                          ...
                          onCheckedChange={checked => {
                            if (checked) toggleFilterValue('country', country);
                            else toggleFilterValue('country', country);
                          }}
                        />
                        <label
                    ...
                    {getUniqueRoles().map(role => (
                      <div key={role} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={checked => {
                            if (checked) toggleFilterValue('role', role);
                            else toggleFilterValue('role', role);
                          }}
                        />
                        <label
                          htmlFor={`role-${role}`}
            ...
            <CardContent className="px-4 py-2">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select
                    onValueChange={value => toggleFilterValue('country', value)}
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={String(t('filters.selectCountry'))}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      {getUniqueCountries().map(country => (
